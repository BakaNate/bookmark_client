...
export async function postNewImgBookmark (url) {
  let res;

  await axios.post(`${servAddr}bookmark/img/`, {url})
    .then((response) => {res = response.data; return response.status})
    .catch((err) => err)
  return res;
}

export async function postNewVideoBookmark(url) {
  let res;

  await axios.post(`${servAddr}bookmark/video/`, {url})
    .then((response) => {res = response.data; return response.status})
    .catch((err) => err)
  return res;
}
